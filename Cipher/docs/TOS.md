# Terms of Service

## 1. Introduction
By using Cipher, you agree to comply with these Terms of Service. If you do not agree, please do not use the bot.

## 2. Usage
- You must follow Discord's [Terms of Service](https://discord.com/terms) and [Community Guidelines](https://discord.com/guidelines).
- You may not use Cipher for illegal activities, harassment, or abuse.
- The bot's commands and features are subject to change without prior notice.

## 3. Data Collection and Privacy
See our [Privacy Policy](https://github.com/Wickslynx/Cipher/blob/main/Cipher/docs/privacy-policy.md) for details on what data we collect and how it is used.

## 4. Liability
Cipher is provided "as is" without any guarantees. We are not responsible for any issues caused by its use.

## 5. Updates and Modifications
We reserve the right to update these Terms at any time. Continued use of the bot implies acceptance of any modifications.
